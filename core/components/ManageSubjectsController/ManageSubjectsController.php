...
class ManageSubjectsController {

    protected $db;

    public function __construct($db) {
        $this->db = $db;
        $this->view = "ManageSubjectsController";
    }

    public function list(){
        $data["list"] = $this->db->Select("select * from subjects where deleted = 0  ", array() );
    ...
    public function source() {
        ...
        if($action == "edit" && ($id != '' || $id != 'undefined') ) {
            $result = $this->db->Select("select * from subjects where id = ?", array($id) )[0];
            $d["details"] = $result;
        }
    ...
    public function afterSubmit(){
        ...
        if(isset($id)) {
            ...
            $this->db->Update("update subjects SET name = ? , code = ?  WHERE id = ? ",
             array($name, $code, $id  ));
            ...
            $this->db->insertRequestBatchRquest($data,'subjects', false);
    ...
    public function delete(){
        ...
        $this->db->Update("update subjects SET deleted = 1 WHERE id = ? ", array( $id) );
